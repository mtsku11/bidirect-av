...
Status: in progress. The original quick-start pair now lives as `Lockup loop`: feedback centroid to visual position plus feedback brightness to audio gain at depth `0.20`. An alternative `Explorer loop` has been added: feedback centroid to visual position plus feedback hue to audio slit position at depth `0.15`. Testing is logged in `test-sessions/2026-05-07-minimal-loop.md`, `test-sessions/2026-05-07-comparative-minimal-loop.md`, `test-sessions/2026-05-07-explorer-loop-screen.md`, `test-sessions/2026-05-07-explorer-loop-post-fix.md`, `test-sessions/2026-05-07-explorer-second-leg-screen.md`, and `test-sessions/2026-05-08-audio-leg-and-hybrid-screen.md`. The old pair remains a bounded lockup demo. The hue-based Explorer loop survives only on `Marquee + Pulse` and `Bouncers + Pulse` under the current cleared-smear protocol. Lower-depth motion/brightness substitutions, an amplitude-based audio-leg swap, and a weak source-amplitude hybrid assist all failed to broaden that coverage, so the next explorer slice should add a new route or derived feature rather than keep rescreening the current family.
