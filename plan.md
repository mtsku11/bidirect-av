...
Status: in progress. The original quick-start pair now lives as `Lockup loop`: feedback centroid to visual position plus feedback brightness to audio gain at depth `0.20`. An alternative `Explorer loop` has been added: feedback centroid to visual position plus feedback hue to audio slit position at depth `0.15`. Testing is logged in `test-sessions/2026-05-07-minimal-loop.md`, `test-sessions/2026-05-07-comparative-minimal-loop.md`, `test-sessions/2026-05-07-explorer-loop-screen.md`, `test-sessions/2026-05-07-explorer-loop-post-fix.md`, and `test-sessions/2026-05-07-explorer-second-leg-screen.md`. The old pair remains a bounded lockup demo. The hue-based Explorer loop survives only on `Marquee + Pulse` and `Bouncers + Pulse` under the current cleared-smear protocol, and lower-depth motion/brightness substitutions do not outperform it, so the next explorer candidate likely needs a different audio-feedback leg or a hybrid assist.
