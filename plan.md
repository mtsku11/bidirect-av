...
Status: in progress. The original quick-start pair now lives as `Lockup loop`: feedback centroid to visual position plus feedback brightness to audio gain at depth `0.20`. The current `Explorer loop` now uses a newly added route: feedback audio spread to visual slit width at depth `0.18`, plus feedback visual hue to audio slit position at depth `0.15`. Testing is logged in `test-sessions/2026-05-07-minimal-loop.md`, `test-sessions/2026-05-07-comparative-minimal-loop.md`, `test-sessions/2026-05-07-explorer-loop-screen.md`, `test-sessions/2026-05-07-explorer-loop-post-fix.md`, `test-sessions/2026-05-07-explorer-second-leg-screen.md`, `test-sessions/2026-05-08-audio-leg-and-hybrid-screen.md`, and `test-sessions/2026-05-08-spread-width-explorer-screen.md`. The old pair remains a bounded lockup demo. The new spread+hue Explorer loop materially broadens coverage to `Marquee` and `Bouncers` with both Pad and Pulse, but `Cars + Pad` and `Cars + Pulse` still lock, so the remaining question is now whether `Cars` needs tuning or its own alternate exploratory preset.
