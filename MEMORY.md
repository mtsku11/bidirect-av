...
- Current finding: the old brightness pair is a reliable bounded lockup demo. Fresh-start reruns on 2026-05-08 narrowed the current spread+hue Explorer pair to one reproducible built-in reference case: `Marquee + Pad`. `Marquee + Pulse` and `Bouncers + Pad` ran away, `Bouncers + Pulse` failed its repeat, `Walker` failed, and `Cars` remains outside the supported Explorer family.
...
- Current next step: re-tune or replace the current Explorer quick-start so it reproduces on more than one built-in reference case, then move to the external challenge suite. Only revisit `Cars` after adding a new feedback-capable route or different control surface.
