...
- Current source determinism: built-in `Pad` detune modulation and `Pulse` note scheduling now reset from fixed seeds for fresh-start screening.
...
- Current finding: the old brightness pair is a reliable bounded lockup demo. After fixing built-in source determinism and adding a Pulse-specific Explorer starting point, the current spread+hue Explorer pair now reproduces on `Marquee + Pulse`, but Pad-based and `Bouncers`-based coverage still fail the reproducibility threshold. `Walker` failed, and `Cars` remains outside the supported Explorer family.
...
- Current next step: continue re-tuning or replace the current Explorer quick-start so it reproduces on more than one built-in reference case, then move to the external challenge suite. Only revisit `Cars` after adding a new feedback-capable route or different control surface.
