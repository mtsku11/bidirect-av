...
- Current next step: move to the external challenge suite now that the current Explorer quick-start reproduces across the seeded `Marquee` and `Bouncers` reference cases. Only revisit `Cars` after adding a new feedback-capable route or different control surface.
