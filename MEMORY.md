...
- Current finding: the old brightness pair is a reliable bounded lockup demo. The newer spread+hue Explorer pair is verified as mobile on `Marquee + Pad`, `Marquee + Pulse`, `Bouncers + Pad`, and `Bouncers + Pulse`, but `Cars + Pad` and `Cars + Pulse` still lock.
...
- Current next step: decide whether to tune the new spread+hue Explorer pair for `Cars` or accept a separate alternate exploratory preset for that scene family.
